...
const dcAdmin = getDataConnect({ location: "us-central1", serviceId: "app" }, admin);

function loadFile(name: string) {
  return JSON.parse(readFileSync(`generated-data/${name}.json`, { encoding: "utf8" }));
}

const { movies } = loadFile("movies");
const { actors } = loadFile("actors");
const { reviews } = loadFile("reviews");
...
async function populateMovies() {
  const moviesData = movies.map((movie: any) => ({
    ...
    tags: movie.tags,
    embedding_embed: { model: "text-embedding-005", text: `${movie.title}: ${movie.description}` },
  }));
...
async function populateReviews() {
  ...
  const watchData = reviews.map((review: any) => ({
    movieId: review.movieId,
    userUid: review.username,
    format: Math.random() > 0.5 ? "home" : Math.random() > 0.8 ? "theater" : "theater-premium",
    watchDate: watchTime,
...
async function populateWatches() {
